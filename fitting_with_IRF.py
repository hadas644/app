...
st.write("max difference:", np.max(np.abs(data_x - irf_x[:len(data_x)])))
st.write("data max:", data_x.max())
st.write("irf max:", irf_x.max())
st.write("any above:", np.any(data_x > irf_x.max()))
st.write("any below:", np.any(data_x < irf_x.min()))
